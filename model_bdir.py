...
class BIDIRDebater(DebaterBase):
    ...
    def define_model():
        ...
        model = Sequential()
        model.add(layers.Embedding(cf.MAX_WORDS, cf.LAYER_SIZE, input_length=cf.MAX_LEN))
        model.add(layers.Bidirectional(layers.LSTM(cf.LAYER_SIZE, dropout=cf.DROPOUT)))
